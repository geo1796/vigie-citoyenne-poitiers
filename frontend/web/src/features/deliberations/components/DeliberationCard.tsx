...
function VoteResult({ deliberation: d }: Props) {
  if (isUnanime(d)) {
    return <span className="text-sm text-muted-foreground">Adoptée à l'unanimité</span>;
  }
  if (isAdoptee(d)) {
...
function VoteBar({ deliberation: d }: Props) {
  ...
  if (isUnanime(d)) {
    return <div className="h-1 w-full rounded-full bg-chart-3 opacity-40" aria-hidden="true" />;
  }
    ...
    >
      <Card>
        <CardHeader className="space-y-3">
          ...
          <h3 className="text-lg font-semibold leading-snug tracking-tight">{d.delibObjet}</h3>
        </CardHeader>
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
              ...
              {!isUnanime(d) && (
                <div className="text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">{d.votePour}</span> pour
                  {' · '}
                  <span className="font-medium text-foreground">{d.voteContre}</span> contre
                  {' · '}
                  <span className="font-medium text-foreground">{d.voteAbstention}</span> abstention
                  {d.voteAbstention > 1 ? 's' : ''}
                  {' · '}
