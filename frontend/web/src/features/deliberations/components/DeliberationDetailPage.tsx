...
import { collectiviteLabels, instanceLabels, isAdoptee, isUnanime } from '../model';

function DocumentsSection({ documents }: { documents: DeliberationDocument[] }) {
      ...
      <Dialog open={active !== null} onOpenChange={(open) => !open && setActive(null)}>
        <DialogContent className="flex h-[90vh] w-[95vw] max-w-5xl flex-col gap-0 p-0 sm:max-w-5xl">
          <div className="flex items-center justify-start gap-4 border-b border-border py-3 pl-4 pr-3">
            <DialogTitle className="truncate text-sm font-medium">{active?.label}</DialogTitle>
          ...
          {active && (
            <iframe src={active.url} title={active.label} className="min-h-0 flex-1 bg-muted" />
          )}
        </DialogContent>
      </Dialog>
    </section>
...
function VoteResultHeading({ deliberation: d }: { deliberation: Deliberation }) {
  if (isUnanime(d)) {
    return (
      <div className="text-2xl font-semibold text-muted-foreground">Adoptée à l'unanimité</div>
    );
  }
  if (isAdoptee(d)) {
...
function VoteBarLarge({ deliberation: d }: { deliberation: Deliberation }) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {pctPour > 0 && <div className="h-full bg-chart-3" style={{ width: `${pctPour}%` }} />}
        {pctContre > 0 && <div className="h-full bg-chart-5" style={{ width: `${pctContre}%` }} />}
        {pctAbstention > 0 && (
          <div className="h-full bg-muted-foreground/40" style={{ width: `${pctAbstention}%` }} />
        )}
      </div>

      <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm">
        <div>
          <span className="mr-2 inline-block size-2 rounded-full bg-chart-3" aria-hidden="true" />
          <span className="font-semibold">{d.votePour}</span>{' '}
          <span className="text-muted-foreground">pour ({Math.round(pctPour)}%)</span>
        </div>
        <div>
          <span className="mr-2 inline-block size-2 rounded-full bg-chart-5" aria-hidden="true" />
          <span className="font-semibold">{d.voteContre}</span>{' '}
          <span className="text-muted-foreground">contre ({Math.round(pctContre)}%)</span>
        </div>
        <div>
          <span
          ...
          <span className="text-muted-foreground">
            abstention{d.voteAbstention > 1 ? 's' : ''} ({Math.round(pctAbstention)}%)
          </span>
        </div>
      </div>
    </div>
...
export function DeliberationDetailPage() {
  ...
  const { data } = useSuspenseQuery(deliberationsQueries.detail(deliberationId));
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <section className="mt-10 space-y-6">
        <h2 className="text-xs uppercase tracking-wider text-muted-foreground">Résultat du vote</h2>
        ...
        <div className="border-t border-border pt-4 text-sm text-muted-foreground">
          <span className="font-medium text-foreground">{data.deliberation.voteReel}</span> votants
          présents sur{' '}
          <span className="font-medium text-foreground">{data.deliberation.voteEffectif}</span> élus
          {/* {tauxPresence !== null && (
						<>
							{" · "}
