...
export function DeliberationsFilter({ params }: Props) {
  ...
  return (
    <div className="space-y-6 py-4">
      ...
      {/* Tri */}
      <div className="space-y-2">
        <Label className="text-xs uppercase tracking-wider text-muted-foreground">Tri</Label>
        <ToggleGroup
          value={[sortValue]}
      ...
      <div className="space-y-2">
        ...
        <ToggleGroup
          ...
          onValueChange={(value: string[]) =>
            updateSearch({
              collectivites: value.length ? (value as Collectivite[]) : undefined,
            })
          }
          variant="outline"
          size="sm"
        ...
        >
          {collectivites.map((c) => (
            <ToggleGroupItem key={c} value={c} aria-label={collectiviteLabels[c]}>
      ...
      <div className="space-y-3">
        <Label className="text-xs uppercase tracking-wider text-muted-foreground">Instances</Label>
        <div className="space-y-2">
          {instances.map((i) => {
              ...
              <div key={i} className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(value) => {
                    ...
                    const next = value === true ? [...current, i] : current.filter((x) => x !== i);
                    updateSearch({
      ...
      {hasAnyFilter && (
        ...
        >
          ...
          {activeCount > 0 && (
            <span className="ml-auto rounded-sm bg-muted px-1.5 py-0.5 text-xs">{activeCount}</span>
          )}
        </Button>
      )}
    </div>
