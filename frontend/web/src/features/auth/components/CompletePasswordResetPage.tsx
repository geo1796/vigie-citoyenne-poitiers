...
export function CompletePasswordResetPage({ token, onSuccess }: Props) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      complete.mutate({ token: token, newPassword: value.newPassword }, { onSuccess });
    },
  });
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-sm flex-col justify-center px-4">
      <div className="mb-6 space-y-2">
        ...
        <h1 className="text-2xl font-semibold tracking-tight">Choisissez un mot de passe</h1>
      </div>
      ...
      >
        ...
        <Button type="submit" className="w-full mt-6" disabled={complete.isPending}>
