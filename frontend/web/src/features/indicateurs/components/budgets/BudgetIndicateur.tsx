...
export function BudgetIndicateur({ observations }: Props) {
  ...
  const selected = useMemo(() => {
    if (search.exercice && exercices.some((e) => e.reference === search.exercice)) {
  ...
  if (exercices.length === 0) {
    return <p className="text-muted-foreground">Aucune donnée budgétaire disponible.</p>;
  }
...
function AxeToggle({
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <span className="text-xs uppercase tracking-wider text-muted-foreground">Répartition</span>
      <ToggleGroup
        value={[axe]}
        onValueChange={(value) => {
          const next = value[0];
      ...
      >
        <Tooltip>
          <TooltipTrigger
            render={
              <ToggleGroupItem value="chapitre">{ventilationAxeLabels.chapitre}</ToggleGroupItem>
            }
          />
          <TooltipContent>
            <p>
              Répartition selon le type d'opération comptable : charges de personnel, achats et
              services, subventions versées, impôts et taxes, dotations de l'État… C'est la
              nomenclature par chapitres, imposée par le plan comptable public. Elle répond à la
              question : « en quoi consiste cette dépense ou cette recette ? »
            </p>
          </TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger
            render={
              <ToggleGroupItem value="fonction">{ventilationAxeLabels.fonction}</ToggleGroupItem>
            }
          />
          <TooltipContent>
            <p>
              Répartition selon le domaine auquel l'argent est affecté : enseignement, culture,
              sport, action sociale, aménagement urbain, services généraux… C'est la nomenclature
              fonctionnelle, également normalisée par l'État. Elle répond à la question : « à quoi
              sert cet argent ? »
            </p>
          </TooltipContent>
        </Tooltip>
...
function ExerciceViewRealise({
  ...
  const tauxExecution = totalDepensesBP > 0 ? totalDepenses / totalDepensesBP : null;
  ...
  return (
    <div className="space-y-10">
      <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm">
        <span className="font-medium text-foreground">Exercice non clôturé.</span>{' '}
        <span className="text-muted-foreground">
          Seul le budget primitif voté est disponible — les montants réalisés n'ont pas encore été
          publiés par la collectivité.
        </span>
      </div>
...
function sumRealise(lignes: LigneBudget[], categorie: LigneBudget['categorie']) {
  return lignes
...
function sumBP(lignes: LigneBudget[], categorie: LigneBudget['categorie']) {
  return lignes.filter((l) => l.categorie === categorie).reduce((acc, l) => acc + l.totalBP, 0);
}
