...
export function BudgetDonut({ title, lignes, categorie, axe, useBP }: Props) {
	...
	const total = useMemo(
		() => parts.reduce((acc, p) => acc + p.montant, 0),
		[parts],
	);
							...
							{p.libelle === "Non ventilé" && (
								<Tooltip>
									...
									<TooltipContent className="max-w-xs text-xs">
										Lignes du budget sans information pour cet axe dans l'open
										data source. Elles sont comptabilisées dans le total
										ci-dessus mais ne peuvent pas être ventilées sur cette
										dimension.
									</TooltipContent>
								</Tooltip>
							)}
