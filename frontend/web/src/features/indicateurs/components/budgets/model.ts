...
export const ligneBudgetSectionSchema = z.enum(['fonctionnement', 'investissement']);
...
export function ventilationEcartParPoste(
  ...
  const groupes = new Map<string, { libelle: string; bp: number; realise: number }>();
  ...
  const result: EcartPoste[] = [...groupes.values()].sort((a, b) => b.bp - a.bp);
