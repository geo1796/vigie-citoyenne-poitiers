...
export function IndicateurDetailPage({ indicateurKey }: Props) {
  const { data: observations } = useSuspenseQuery(indicateursQueries.observations(indicateurKey));
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <header className="mb-8">
        <div className="text-xs uppercase tracking-wider text-muted-foreground">Indicateur</div>
        <h1 className="mt-1 text-3xl font-semibold tracking-tight">
          {indicateurKeyLabels[indicateurKey]}
        </h1>
      </header>

      <IndicateurDetail indicateurKey={indicateurKey} observations={observations} />
    </div>
  );
}
